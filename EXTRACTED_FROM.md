...
| | |
|---:|---|
| **137** 파일 | 총 3.3 MiB (이 문서 제외 — 자기 크기는 자기 숫자에 의존하므로 셀 수 없다. 파일 수에는 이 문서가 포함된다) |
| **75** 매니페스트 entry | 이 파일들을 예약한 선언의 개수 |
| 최상위 항목 | `.extraction-layout.json` · `.gitignore` · `CODEOWNERS` · `EXTRACTED_FROM.md` · `README.md` · `artifacts` · `docs` · `fcc_test_contracts` · `packages` · `pyproject.toml` · `scripts` · `tests` |
