...
⚠️ **이 게이트는 커밋된 것만 본다** (실측 2026-09-05, 이 게이트를 쓰다가 드러났다).
`git log` 가 작업트리를 세지 않으므로, 배포되는 파일을 고쳐 놓고 **아직 커밋하지 않은**
상태는 초록이다. 그것이 옳다 — 아직 아무 이력도 만들지 않았으니 태그가 담을 것도 없다.
판정은 첫 커밋 직후에 온다(그리고 pre-push 가 그 뒤에 있다). 이 문장이 없으면 다음 사람이
「고쳤는데 안 붉네」를 게이트가 안 도는 것으로 읽는다.
