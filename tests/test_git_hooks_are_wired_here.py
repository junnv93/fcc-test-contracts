...
class TestTheProbeDoesNotDisarmTheRepositoryItChecks(unittest.TestCase):
    ...
    #    갖지 않기 위해서다. 실측 2026-08-31(platform): 초판은 `_victim` 만 격리를
    #    빠뜨렸고, 훅 안에서는 부모의 `GIT_DIR` 이 **이미** 서 있으므로 피해자를
    #    만들려던 `git init`/`git commit` 이 **진짜 저장소**를 향했다(`core.bare` 가
    #    뒤집혔고, 커밋은 *"nothing to commit, working tree clean"* 으로 실패했다 —
    #    그 트리가 victim 이 아니라 개발자의 워크트리였기 때문이다).
    ...
    def test_the_isolation_is_a_no_op_outside_a_hook(self) -> None:
        ...
        ⚠️ 원래 값을 **복원**한다 — 훅 안에서 돌 때 이 검사가 부모의 `GIT_DIR` 을
        지워 버리면, 뒤따르는 검사가 「훅 문맥」이 아닌 곳에서 도는 것이 된다.
        """
        saved = {
