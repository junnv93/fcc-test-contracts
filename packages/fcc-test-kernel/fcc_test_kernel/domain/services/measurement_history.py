...
**출신**: `fcc_test_platform.domain.services.measurement_history`. 2026-09-07 에
커널로 상류 수리됐다.

⚠️ **`test_plan_snapshot`·`notification_types` 와 근거가 «다르다».** 그 둘은
platform 배포판 안에 소비자가 없어서(자리가 틀려서) 옮겼다. 이 모듈은 **양쪽이
진짜로 쓴다**:

    platform 레포   domain/services/rekey_dry_run.py  — ConditionFieldSet 과
                    해시 계산기 둘을 실제 로직에서 쓴다 (re-key dry-run)
    모노레포 GUI    5개 모듈 — test_plan_publish_service · condition_identity ·
                    test_plan_materialization · test_plan_publish ·
                    measurement_history_store

즉 「자리가 틀렸다」가 아니라 **공유 서비스가 한쪽 배포판에만 있었다**는 것이다.
그래서 커널이 옳은 자리다 — 그리고 이 모듈은 **이미** 커널을
(`central_session_identity.measurement_target_key`) import 하고 있었으므로, 올리면
의존이 늘지 않고 줄어든다.

⚠️ **이것은 해시 SSOT 다.** `condition_hash` 는 measurement identity 이고, 같은
구현이 두 자리에 있으면 두 자리가 갈리는 날 **같은 조건이 다른 해시**를 낳는다 —
DTO 사본과 달리 이 사본은 데이터를 오염시킨다. platform 쪽 사본의 처리(철수 또는
이 모듈로의 위임)는 그 저장소의 후속 판정이다.
"""
