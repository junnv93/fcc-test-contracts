...
> 「이 레인이 자기 OpenAPI 를 발행한다 — 조립기가 저쪽에 있어 다섯 사본이 낡았다」),
> provider 어휘 오염 같은 계약 결함도 여기서 고칩니다(`4740cd0`).
>
> ⚠️ **다만 배송 체계는 아직 살아 있고, 파일마다 다릅니다.**
> `.extraction-layout.json` 에 등재된 **140개 경로**(`src` 60 · `tests` 34 · `docs` 20 ·
> `packages` 10 · `packaging` 9 · `scripts` 7)는 여전히 모노레포에서 생성되어 배송되며,
> **그 파일을 고치면 배송이 거부**합니다. 등재되지 않은 것(이 README 포함)은
> **여기서 자유롭게 작성·수정**합니다.
>
...
`.extraction-layout.json` 에 등재된 **140개 경로**는 배송이 관리합니다 — 고치면 배송이
그 파일을 이름으로 대며 거부합니다. 그 밖의 것(이 README 포함)은 **여기서 자유롭게
작성·수정**합니다.

### 2. git history 는 **있습니다** (2026-09-05 정정)

이 레포는 **76개 커밋**을 갖고 있고 여기서 개발이 이뤄집니다 — 자기 OpenAPI 발행
(`d83ebee`), 계약 결함 수정(`4740cd0`) 모두 이 레포의 커밋입니다.
다만 **모노레포의 history 는 이전되지 않았습니다.**

> ⚠️ 옛 판은 「첫 커밋이 이 레포의 전부」·「`git log` 가 비어 보이는 것이 정상」이라고
> 적었습니다. 지금은 사실이 아닙니다.
...
> **당신이 무언가를 깨뜨린 것이 아닙니다.** 2026-09-05 실측 **29건**이 `delivered_test_run_baseline.json` 에 이름으로 등재돼 있습니다.
> (형제 레인 `fcc-test-platform` 은 이 집합이 **비어 있습니다** — 레인마다 다릅니다.)
...
이 레인은 정식 분리되어 여기서 개발합니다. 다만 **배송이 관리하는 140개 경로**는
