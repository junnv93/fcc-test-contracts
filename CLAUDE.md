...
### 🔴 그래서 «평시에도» 무엇이 강제되고 무엇이 안 되는지 — 정확히

위 문단을 「비상시 조항」으로 읽지 마십시오. **평시에도 그렇습니다.**

실측(2026-09-07): 이 레포의 협업자는 **1명이고 그 1명이 admin** 입니다
(`role=admin`). 그러므로 「관리자는 우회할 수 있다」는 곧
**「사실상 아무나, 언제든 우회할 수 있다」** 입니다.

| | 평시에도 강제되나 |
|---|---|
| `main` 직접 push 차단 | ✅ **예** — 우회 불가 |
| `main` 에 force push · 삭제 | ✅ **예** — 우회 불가 |
| `lane-check` 이 초록일 것 | ❌ **아니오** — 관리자가 언제든 넘긴다 |
| 리뷰 승인 | ❌ 애초에 0 을 요구한다 |

> **즉 이 레포에서 보호가 «실제로» 하는 일은 「PR 을 거치게 하는 것」 하나입니다.**
> `lane-check` 은 그 PR 에 붙는 **조언**이고, 그것이 «의도된 효과»입니다 —
> 위의 태그 레인 논거가 그 대가로 받아들인 것입니다.
>
> ⚠️ 「red 면 서버가 막는다」는 **이 레포에서 거짓입니다.** 이 문장을 지우지
> 마십시오 — 지우면 다음 사람이 틀린 보장을 믿습니다.
>
> ⚠️ 협업자가 **둘 이상**이 되는 날 이 계산이 바뀝니다. 그때
> `required_approving_review_count` 를 1 로 올리고 `enforce_admins` 를 다시
> 판단하십시오 — 그때는 「관리자만 우회」가 「전원 우회」와 달라집니다.
