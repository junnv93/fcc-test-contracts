...
### ✅ `enforce_admins` 를 **켰습니다** (2026-09-07 13:4x) — 이제 platform 과 «같습니다»

| | `fcc-test-contracts` | `fcc-test-platform` |
|---|---|---|
| `enforce_admins` | ✅ **`true`** | ✅ `true` |

**관리자도 `lane-check` 을 우회하지 못합니다.** 즉 **「red 면 서버가 막는다」가
이 레포에서도 참이 됐습니다.**

<!-- 정정 이력 — 지우지 마세요. 이 절의 옛 판을 근거로 「여기서는 서버가 안 막는다」고
     적은 문서가 두 레포에 여럿 있었고, 그것들도 같은 날 함께 고쳤습니다. -->
> 🔄 **결정 이력 — 하루 만에 뒤집혔습니다.**
>
> **옛 판 (2026-09-07 오전, `false`):**
>
...
> >
> > 그리고 그 대가를 이렇게 적었습니다 — *「이 레포에서 보호가 «실제로» 하는 일은
> > 「PR 을 거치게 하는 것」 하나입니다. … **「red 면 서버가 막는다」는 이 레포에서
> > 거짓입니다.**」*
>
> **왜 뒤집혔나:** 그 논거는 **하나의 전제** 위에 서 있었습니다 —
> *「러너 할당량 문제의 원인이 아직 해소되지 않았다」.*
> **2026-09-07 운영자 판정: 그 전제는 해소됐습니다.** 두 레포가 `public` 이 되어
> 표준 러너를 무과금으로 받고, 최근 런이 전부 `success` 이며 `runner_name` 이
> 채워집니다. 전제가 죽으면 그 위의 결론도 죽습니다 — **결정을 뒤집기 전에
> 근거의 진위를 재라**는 것이 이 저장소의 규율이고, 이번에는 근거가 먼저 죽었습니다.
>
> ⚠️ **되돌리는 조건을 여기 적어 둡니다.** 할당량이 다시 차서 이 레인의 `lane-check`
> 이 **러너를 못 받으면**, 그때는 이 값이 다시 공급 사슬을 멈춥니다. 그날 먼저 볼 것은
> `conclusion` 이 아니라 **`runner_name`** 입니다 —
> `gh api repos/junnv93/fcc-test-contracts/actions/runs/<id>/jobs`.
> 러너 부재가 확인되면 **운영자 승인 아래** 다음으로 되돌립니다:
> `gh api -X DELETE repos/junnv93/fcc-test-contracts/branches/main/protection/enforce_admins`

### 그래서 «오늘» 무엇이 강제되고 무엇이 안 되는지 — 정확히

| | 강제되나 | 비고 |
|---|---|---|
| `main` 직접 push 차단 | ✅ **예** | 관리자도 불가 |
| `main` 에 force push · 삭제 | ✅ **예** | 관리자도 불가 |
| **`lane-check` 이 초록일 것** | ✅ **예** | 🆕 2026-09-07 13:4x 부터. 관리자도 못 넘긴다 |
| 리뷰 승인 | ❌ **아니오** | `required_approving_review_count: 0` |
| CODEOWNERS 리뷰 | ❌ 아니오 | `require_code_owner_reviews: false` |

> ⚠️ **남은 것은 「승인」 축 하나입니다.** 검사는 서버가 막지만 **사람의 승인은
> 아무것도 요구하지 않습니다.** 「검사가 통과했으니 승인된 것」이 아닙니다.
>
> ⚠️ 협업자가 **둘 이상**이 되는 날 `required_approving_review_count` 를 1 로 올릴지
> 판단하십시오. ⚠️ **순서를 뒤집지 마십시오** — 수락 «전»에 올리면 자기 PR 자기 승인
> 금지로 아무것도 머지할 수 없습니다. 판정은 `invitations` 가 아니라
> **`collaborators`** 로 하십시오. 그 둘은 서로 다른 엔드포인트입니다.
